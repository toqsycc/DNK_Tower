...
void LEDstrip::deathLight(int dval = 100)
{
  for (byte i = 0; i < 8; i++)
  {
  ...
  FastLED[0].showLeds();
  delay(dval); // Здесь дилеями можно воспользоваться, т.к. башня больше не регистрирует новые попадания
  for (byte i = 0; i < 8; i++)
  {
    leds[i] = CRGB::Black;
  }
